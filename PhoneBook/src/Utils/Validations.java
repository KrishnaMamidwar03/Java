...
public class Validations {
	...
	public static PhoneBook ValidInputs(long number, String name, String dob, String email, List<PhoneBook> list)
			throws PhoneException {
		checkNumber(number);// checking phone number length
		checkDupNum(number, list);
	...
	public static void checkNumber(long number) throws PhoneException {
		String num = "^[0-9]*$";
		// using valueOf cause data type is not same
		if (!Pattern.matches(num, String.valueOf(number)) || String.valueOf(number).length() != 10) {
			throw new PhoneException("Number is not valid.");
		}
	}

	public static void checkDupNum(long number, List<PhoneBook> list) throws PhoneException {
		for (PhoneBook b : list) {
			if ((b.getNumber())==(number))
				throw new PhoneException("number is already existed!!");
